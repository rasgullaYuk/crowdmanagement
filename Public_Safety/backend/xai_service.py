...
def generate_xai_dashboard_image(video_path, weights_path):
    ...
    # RESIZE to reduce memory usage (Max width 640)
    w_percent = (640 / float(img_pil.size[0]))
    h_size = int((float(img_pil.size[1]) * float(w_percent)))
    img_pil = img_pil.resize((640, h_size), Image.Resampling.LANCZOS)
    ...
    # Reduce steps if CPU to prevent timeout (e.g. 10 instead of 15)
    # Set internal_batch_size=1 to prevent OOM
    ig = IntegratedGradients(model_wrapper)
    ig_attr = ig.attribute(input_tensor, n_steps=10, internal_batch_size=1)
